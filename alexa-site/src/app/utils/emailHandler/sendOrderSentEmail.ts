...
export const sendOrderSentEmail = (userData: UserType, orderId: string, orderData: OrderType) => {
    try {
        if (!userData) {
            throw new Error('Usuário não encontrado ao tentar enviar email de confirmação de pedido enviado');
        }

        if (!orderData) {
            throw new Error('Dados do pedido não encontrados ao tentar enviar email de confirmação de pedido enviado');
        }
        ...
        <body style="margin: 0; padding: 0; background-color: #FAF9F6; font-family: 'Helvetica Neue', Arial, sans-serif;">
            <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; padding: 0 0 40px 0; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
                ...
                <div style="padding: 30px 40px; background-color: #ffffff;">
                    ...
                    <p style="color: #333333; font-size: 16px; line-height: 1.5;">
                        Informamos que o seu pedido #${orderId} foi enviado.
                    </p>
        ...
        const msg = {
            ...
            subject: `Alexa Semijoias - Pedido enviado #${orderId}`,
            html: emailHtml,
        };
