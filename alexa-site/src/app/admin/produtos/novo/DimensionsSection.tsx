...
export default function DimensionsSection({ state, handlers }: DimensionsSectionProps) {
    const [dimensionsState, setDimensionsState] = useState({
        altura: state.dimensions && state.dimensions.altura ? (state.dimensions.altura * 100) : 0,
        comprimento: state.dimensions && state.dimensions.comprimento ? (state.dimensions.comprimento * 100) : 0,
        largura: state.dimensions && state.dimensions.largura ? (state.dimensions.largura * 100) : 0,
        peso: state.dimensions && state.dimensions.peso ? (state.dimensions.peso * 100) : 0,
    });
    ...
    return (
        <section className="flex flex-col gap-2 p-2 py-4 border rounded-md bg-white w-full">
            ...
            <div className='flex flex-wrap justify-center w-full gap-4 text-xs'>
                <InputSection handleChange={ handlers.handleDimensionsChange } stateToBeChange={ dimensionsState }/>
            </div>
        </section>
    );
}
